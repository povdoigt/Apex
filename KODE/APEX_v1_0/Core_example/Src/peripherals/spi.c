...
#ifdef SPI1
  osSemaphoreId_t spi1_use_semaphore;
  osSemaphoreId_t spi1_dma_semaphore;
#endif
#ifdef SPI2
  osSemaphoreId_t spi2_use_semaphore;
  osSemaphoreId_t spi2_dma_semaphore;
#endif
} spi_semaphores_t;
...
osSemaphoreId_t spi_use_semaphore_get(SPI_HandleTypeDef *hspi) {
	...
	if (hspi->Instance == SPI1) {
		return spi_semaphores.spi1_use_semaphore;
	}
#endif
#ifdef SPI2
	if (hspi->Instance == SPI2) {
		return spi_semaphores.spi2_use_semaphore;
	}
#endif
...
#ifdef SPI1
	if (hspi->Instance == SPI1) {
		return spi_semaphores.spi1_dma_semaphore;
	}
#endif
#ifdef SPI2
	if (hspi->Instance == SPI2) {
		return spi_semaphores.spi2_dma_semaphore;
	}
#endif
	return NULL; // Invalid SPI instance
}
...
void Init_spi_semaphores() {
	uint32_t count;
	#ifdef SPI1
	spi_semaphores.spi1_use_semaphore = osSemaphoreNew(1, 1, NULL);
	spi_semaphores.spi1_dma_semaphore = osSemaphoreNew(1, 1, NULL);
	#endif
	#ifdef SPI2
	spi_semaphores.spi2_use_semaphore = osSemaphoreNew(1, 1, NULL);
	spi_semaphores.spi2_dma_semaphore = osSemaphoreNew(1, 1, NULL);
	#endif
}
