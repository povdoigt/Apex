...
sx127x_status_t __sx127x_FSK_OOK_Config(sx127x_FSK_OOK_chip_t *chip, sx127x_base_chip_t *base_chip, sx127x_FSK_OOK_config_t config, sx127x_modulation_t modulation) {
	...
	value = sx127x_FSK_OOK_REG_0D_RX_CONFIG_RROC_OFF	// Restart Rx on collision
		  | sx127x_FSK_OOK_REG_0D_RX_CONFIG_AFCAON_ON	// AFC disabled
	...
	value = sx127x_FSK_OOK_REG_27_SYNC_CONFIG_ARRXM_WAIT
		  | sx127x_FSK_OOK_REG_27_SYNC_CONFIG_PP_AA
...
sx127x_status_t __sx127x_FSK_OOK_SetMode_RTOS(sx127x_FSK_OOK_chip_t *chip, sx127x_FSK_OOK_REG_01_OP_MODE_MODE mode) {
	...
	sx127x_status_t status;
	uint8_t value;
	status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_REG_01_OP_MODE, &value);
	if (status != sx127x_STATUS_OK) { return status; }
	value &= ~sx127x_FSK_OOK_REG_01_OP_MODE_MODE_MSK;
	value |= mode;
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_REG_01_OP_MODE, value);
	if (status != sx127x_STATUS_OK) { return status; }

	return sx127x_STATUS_OK;
}

sx127x_status_t __sx127x_FSK_OOK_Config_RTOS(sx127x_FSK_OOK_chip_t *chip, sx127x_base_chip_t *base_chip, sx127x_FSK_OOK_config_t config, sx127x_modulation_t modulation) {
    if (!chip) { return sx127x_STATUS_ERROR; }
    ...
    // Set the mode to sleep (required to switch from LoRa to FSK or configure FSK)
    status = __sx127x_FSK_OOK_SetMode_RTOS(chip, sx127x_FSK_OOK_REG_01_OP_MODE_MODE_SLEEP);
	if (status != sx127x_STATUS_OK) { return status; }
    ...
    uint8_t bitrate_buf[2] = {
        (uint8_t)((bitrate_reg >> 8) & 0xFF),
        (uint8_t)((bitrate_reg >> 0) & 0xFF) };
    status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_02_BITRATE_MSB, bitrate_buf, 2);
    if (status != sx127x_STATUS_OK) { return status; }
	...
	value = sx127x_FSK_OOK_REG_0D_RX_CONFIG_RROC_OFF	// Restart Rx on collision
		  ...
		  | sx127x_FSK_OOK_REG_0D_RX_CONFIG_AGCAON_ON	// AGC enabled
		  | 0x00; // Default
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_0D_RX_CONFIG, value);
	if (status != sx127x_STATUS_OK) { return status; }

	// Set PA ramp and modulation shaping
	value = config.modShaping
		  | config.paRamp;	// Useful only for FSK modulation 
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_0A_PA_RAMP, value);
	if (status != sx127x_STATUS_OK) { return status; }

	// Set RxBw (Receiver bandwidth)
	uint8_t rxbw_mant, rxbw_exp;
	__sx127x_FSK_OOK_RxBw_to_RegValue(config.RxBw, &rxbw_mant, &rxbw_exp);
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_12_RX_BW, rxbw_mant | rxbw_exp);
	if (status != sx127x_STATUS_OK) { return status; }
	...
	uint8_t preamble_buf[2] = {
		(uint8_t)((config.packetCfg.preamble_len >> 8) & 0xFF),
		(uint8_t)((config.packetCfg.preamble_len >> 0) & 0xFF) };
	status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_25_PREAMBLE_MSB, preamble_buf, 2);
	if (status != sx127x_STATUS_OK) { return status; }
	...
	value = sx127x_FSK_OOK_REG_27_SYNC_CONFIG_ARRXM_WAIT
		  ...
		  | (config.packetCfg.sync_on ? sx127x_FSK_OOK_REG_27_SYNC_CONFIG_SO_ON : sx127x_FSK_OOK_REG_27_SYNC_CONFIG_SO_OFF)
		  | ((config.packetCfg.sync_len - 1) & sx127x_FSK_OOK_REG_27_SYNC_CONFIG_SYNC_SIZE_MSK);
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_27_SYNC_CONFIG, value);
	if (status != sx127x_STATUS_OK) { return status; }

	// Write sync word bytes
	if (config.packetCfg.sync_on && config.packetCfg.sync_len > 0) {
		uint8_t write_len = (config.packetCfg.sync_len > 8) ? 8 : config.packetCfg.sync_len;
		status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_28_SYNC_VALUE_1, config.packetCfg.sync_word, write_len);
		if (status != sx127x_STATUS_OK) { return status; }
	}
	...
	value = (config.packetCfg.variable_length ? sx127x_FSK_OOK_REG_30_PACKET_CONFIG1_PF_VARIABLE : sx127x_FSK_OOK_REG_30_PACKET_CONFIG1_PF_FIXED)
		  ...
		  | (config.packetCfg.crc_on ? sx127x_FSK_OOK_REG_30_PACKET_CONFIG1_CRCON_ON : sx127x_FSK_OOK_REG_30_PACKET_CONFIG1_CRCON_OFF)
		  | sx127x_FSK_OOK_REG_30_PACKET_CONFIG1_AF_OFF;
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_30_PACKET_CONFIG1, value);
	if (status != sx127x_STATUS_OK) { return status; }
	...
	value = sx127x_FSK_OOK_REG_31_PACKET_CONFIG2_DM_PACKET
		  ...
		  | sx127x_FSK_OOK_REG_31_PACKET_CONFIG2_BEACONON_OFF
		  | len[0];
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_31_PACKET_CONFIG2, value);
	if (status != sx127x_STATUS_OK) { return status; }

	value = len[1];
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_32_PAYLOAD_LENGTH, value);
	if (status != sx127x_STATUS_OK) { return status; }
...
sx127x_status_t sx127x_FSK_Config_RTOS(sx127x_FSK_OOK_chip_t *chip, sx127x_base_chip_t *base_chip, sx127x_FSK_OOK_config_t config) {
	...
	if (osSemaphoreAcquire(chip->base_chip->sem_id, osWaitForever) != osOK) { return sx127x_STATUS_ERROR; }

	sx127x_status_t status;

	status = __sx127x_FSK_OOK_Config_RTOS(chip, base_chip, config, sx127x_MODULATION_FSK);
	...
	uint8_t fdev_buf[2] = {
		(uint8_t)((fdev_reg >> 8) & 0x3F), // [15-14] Reserved
		(uint8_t)((fdev_reg >> 0) & 0xFF) };
	status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_04_FDEV_MSB, fdev_buf, 2);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

exit_flag:
	if (osSemaphoreRelease(chip->base_chip->sem_id) != osOK) { return sx127x_STATUS_ERROR; }
...
sx127x_status_t sx127x_OOK_Config_RTOS(sx127x_FSK_OOK_chip_t *chip, sx127x_base_chip_t *base_chip, sx127x_FSK_OOK_config_t config) {
	...
	if (osSemaphoreAcquire(chip->base_chip->sem_id, osWaitForever) != osOK) { return sx127x_STATUS_SEM_ERROR; }

	status = __sx127x_FSK_OOK_Config_RTOS(chip, base_chip, config, sx127x_MODULATION_OOK);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
    ...
    switch (config.mod_config.ook.ookCfg.thresh_type) {
        case SX127X_OOK_THRESH_FIXED:
            // Set fixed threshold
            value = sx127x_FSK_OOK_REG_14_OOK_PEAK_THRESH_TYPE_FIXED;
            status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_14_OOK_PEAK, value);
            if (status != sx127x_STATUS_OK) { goto exit_flag; }
			value = config.mod_config.ook.ookCfg.params.fixed.fixed_thresh;
            status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_15_OOK_FIX, value);
            if (status != sx127x_STATUS_OK) { goto exit_flag; }
            break;
        ...
        case SX127X_OOK_THRESH_AVERAGE:
            ...
            value = config.mod_config.ook.ookCfg.params.average.avg_peak_dec
                  | config.mod_config.ook.ookCfg.params.average.avg_offset
                  | config.mod_config.ook.ookCfg.params.average.avg_filt;
            status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_16_OOK_AVG, value);
            if (status != sx127x_STATUS_OK) { goto exit_flag; }
            break;
    }
...
exit_flag:
	...
	if (osSemaphoreRelease(chip->base_chip->sem_id) != osOK) { return sx127x_STATUS_SEM_ERROR; }
...
sx127x_status_t __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(sx127x_FSK_OOK_chip_t *chip, uint32_t Timeout_delay_ms, bool is_flag_2,
													uint8_t irq_mask, bool invert_mask, uint8_t irq[2]) {
	if (!chip || chip->base_chip->modulation == sx127x_MODULATION_LORA) { return sx127x_STATUS_ERROR; }
	...
	uint32_t t0 = HAL_GetTick();
	while (true) {
		status = sx127x_RegReadMulti_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_3E_IRQ_FLAGS1, irq, 2);
		if (status != sx127x_STATUS_OK) { break; }
		...
		// Check for timeout
		if ((HAL_GetTick() - t0) > Timeout_delay_ms) {
			status = sx127x_STATUS_MODEM_TIMEOUT;
			break;
		}
	}
...
sx127x_status_t sx127x_FSK_OOK_TxSend_RTOS(sx127x_FSK_OOK_chip_t *chip, const uint8_t *data, uint16_t len) {
	...
	// Set the mode to standby
	status = __sx127x_FSK_OOK_SetMode_RTOS(chip, sx127x_FSK_OOK_REG_01_OP_MODE_MODE_STDBY);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Wait for mode ready
	status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, HAL_MAX_DELAY, false, sx127x_FSK_OOK_REG_3E_IRQ_FLAGS1_MODE_READY_MSK, false, values);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Clear FIFO by clearing FIFO Overrun flag
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_OVERRUN_MSK);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	if (chip->config.packetCfg.variable_length) {
		...
		// Variable length packets: first byte is length
		max_size--;
		status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, (uint8_t)(len & 0xFF));
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
	} else {
		// Fixed length packets: get length from config
		len = chip->config.packetCfg.payload_len;
	...
	if (len > max_size) {
		...
		// Prefilled !FIFO
		status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, data, max_size);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
		len -= max_size;
		offset += max_size;
		...
		values[0] = sx127x_FSK_OOK_REG_35_FIFO_THRESH_TX_START_COND_LEVEL
				  | (fifo_half & sx127x_FSK_OOK_REG_35_FIFO_THRESH_FIFO_THRESH_MSK);
		status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_35_FIFO_THRESH, values[0]);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }

		// Set the mode to transmit
		status = __sx127x_FSK_OOK_SetMode_RTOS(chip, sx127x_FSK_OOK_REG_01_OP_MODE_MODE_TX);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }

		// Write data to FIFO in chunks
		while (len > 0) {
			// Wait until FIFO threshold is cleared
			status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, HAL_MAX_DELAY, true, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_LEVEL_MSK, true, values);
			if (status != sx127x_STATUS_OK) { goto exit_flag; }
			// Verify overrun did not occur
			if (values[1] & sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_OVERRUN_MSK) { return sx127x_STATUS_ERROR; }

			chunk_size = (len > fifo_half) ? fifo_half : (uint8_t)len;
			status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, &data[offset], chunk_size);
			if (status != sx127x_STATUS_OK) { goto exit_flag; }
		...
		// Write entire data to FIFO
		status = sx127x_RegWriteMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, data, len);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	// Wait for PacketSent IRQ
	status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, HAL_MAX_DELAY, true, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_PACKET_SENT_MSK, false, values);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
...
sx127x_status_t sx127x_FSK_OOK_RxReceive_RTOS(sx127x_FSK_OOK_chip_t *chip, uint8_t *data, uint16_t *len_ptr) {
	if (!chip || chip->base_chip->modulation == sx127x_MODULATION_LORA || !data) { return sx127x_STATUS_ERROR; }
	...
	status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_35_FIFO_THRESH, values);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	values[0] &= ~sx127x_FSK_OOK_REG_35_FIFO_THRESH_FIFO_THRESH_MSK;
	values[0] |= (fifo_half & sx127x_FSK_OOK_REG_35_FIFO_THRESH_FIFO_THRESH_MSK);
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_FSK_OOK_REG_35_FIFO_THRESH, values[0]);
	...
	// Set the mode to Rx
	status = __sx127x_FSK_OOK_SetMode_RTOS(chip, sx127x_FSK_OOK_REG_01_OP_MODE_MODE_RX);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Wait for FifoEmpty clear
	status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, RxTimeout_delay_ms, true, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_EMPTY_MSK, true, values);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// If variable length, read first byte as length
	if (chip->config.packetCfg.variable_length) {
		status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, values);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
		*len_ptr = values[0];
		len = 0;
		len = *len_ptr; // Overwrite len with received length
	...
	if (nb_chunks_adjusted > 0) {
		for (uint8_t c = 0; c < nb_chunks_adjusted; c++) {
			// Wait until FIFO threshold is reached
			status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, RxTimeout_delay_ms, true, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_LEVEL_MSK, false, values);
			if (status != sx127x_STATUS_OK) { goto exit_flag; }
			// Verify overrun did not occur
			if (values[1] & sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_FIFO_OVERRUN_MSK) { return sx127x_STATUS_ERROR; }
			// Read chunk from FIFO
			status = sx127x_RegReadMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, &data[i], fifo_half);
			if (status != sx127x_STATUS_OK) { goto exit_flag; }
			i += fifo_half;
		}
	}
	...
	// Wait for PayloadReady flag
	status = __sx127x_FSK_OOK_RxReceive_IRQ_RTOS(chip, RxTimeout_delay_ms, true, sx127x_FSK_OOK_REG_3F_IRQ_FLAGS2_PAYLOAD_READY_MSK, false, values);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	if (remaining_adjusted > 0) {
		...
		// Values already available in FIFO
		status = sx127x_RegReadMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, &data[i], remaining_adjusted);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
		i += remaining_adjusted;
	}
