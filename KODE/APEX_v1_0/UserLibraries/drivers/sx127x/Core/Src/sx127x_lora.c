...
sx127x_status_t sx127x_LORA_RxReceive(sx127x_LORA_chip_t *chip, uint8_t *data, uint8_t *len) {
	...
	*len = packetLength;
	...
	status = sx127x_RegReadMulti(chip->base_chip, sx127x_REG_00_FIFO, data, packetLength);
	if (status != sx127x_STATUS_OK) { return status; }
...
static sx127x_status_t __sx127x_LORA_IsTxDone_RTOS(sx127x_LORA_chip_t *chip, bool *isDone) {
	if (!chip || chip->base_chip->modulation != sx127x_MODULATION_LORA) { return sx127x_STATUS_ERROR; }
	sx127x_status_t status;
	uint8_t irq_flags;

	// Read the IRQ flags
	status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_12_IRQ_FLAGS, &irq_flags);
	if (status != sx127x_STATUS_OK) { return status; }
	...
	*isDone = irq_flags & sx127x_LORA_IRQ_FLAG_TX_DONE_MSK;
	...
	// Clear the TX done flag
	if (*isDone) {
		status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_12_IRQ_FLAGS, sx127x_LORA_IRQ_FLAG_TX_DONE_MSK);
		if (status != sx127x_STATUS_OK) { return status; }
	}
...
sx127x_status_t sx127x_LORA_Config_RTOS(sx127x_LORA_chip_t *chip, sx127x_base_chip_t *base_chip, sx127x_LORA_config_t config) {
	...
	// Set the mode to sleep (need to be in sleep mode to set LoRa mode)
	status = __sx127x_LORA_SetMode_RTOS(chip, sx127x_LORA_REG_01_OP_MODE_MODE_SLEEP);
	...
	// Set the fifo rx and tx base address to 0x00
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_0F_FIFO_RX_BASE_ADDR, 0x00);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_0E_FIFO_TX_BASE_ADDR, 0x80);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	value = config.bandwidth		// Bandwidth
		  | config.codingRate		// Coding Rate
		  | config.implicitHeader;	// Implicit Header mode
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_1D_MODEM_CONFIG1, value);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	value = config.spreadingFactor						// Spreading Factor
		  | sx127x_LORA_REG_1E_MODEM_CONFIG2_TXCM_OFF	// Normal Tx mode
		  | (config.crcEnabled ? sx127x_LORA_REG_1E_MODEM_CONFIG2_RPC_ON : sx127x_LORA_REG_1E_MODEM_CONFIG2_RPC_OFF); // CRC
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_1E_MODEM_CONFIG2, value);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Set the Modem Config3
	value = sx127x_LORA_REG_26_MODEM_CONFIG3_LDRO_OFF	// LowDataRateOptimize disabled
		  | sx127x_LORA_REG_26_MODEM_CONFIG3_AGCAO_ON;	// LNA gain set by AGC loop
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_26_MODEM_CONFIG3, value);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
...
sx127x_status_t sx127x_LORA_TxSend_RTOS(sx127x_LORA_chip_t *chip, const uint8_t *data, uint8_t len) {
	...
	// Set the mode to standby
	status = __sx127x_LORA_SetMode_RTOS(chip, sx127x_LORA_REG_01_OP_MODE_MODE_STDBY);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Set the fifo pointer to TxBaseAddr (0x80)
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_0D_FIFO_ADDR_PTR, 0x80);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	// Set the payload length
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_22_PAYLOAD_LENGTH, len);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Set the mode to transmit
	status = __sx127x_LORA_SetMode_RTOS(chip, sx127x_LORA_REG_01_OP_MODE_MODE_TX);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Check TxDone flag
	bool isDone = false;
	while (!isDone) {
		status = __sx127x_LORA_IsTxDone_RTOS(chip, &isDone);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
	}
...
sx127x_status_t sx127x_LORA_RxReceive_RTOS(sx127x_LORA_chip_t *chip, uint8_t *data, uint8_t *len) {
	...
	// Set the mode to Rx continuous
	status = __sx127x_LORA_SetMode_RTOS(chip, sx127x_LORA_REG_01_OP_MODE_MODE_RX_CONTINUOUS);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Check for irq flags
	uint32_t t0 = HAL_GetTick();
	while (true) {
		status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_12_IRQ_FLAGS, &value);
		if (status != sx127x_STATUS_OK) { goto exit_flag; }
		if (value & sx127x_LORA_IRQ_FLAG_RX_DONE_MSK) {
			break;
		}
	...
	// Clear the RX done flag
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_12_IRQ_FLAGS, sx127x_LORA_IRQ_FLAG_RX_DONE_MSK);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	...
	// Read packet length
	status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_13_RX_NB_BYTES, &packetLength);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
	*len = packetLength;

	// Set FIFO address to current RX address
	status = sx127x_RegRead_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_10_FIFO_RX_CURRENT_ADDR, &value);
	if (status != sx127x_STATUS_OK) { return status; }
	status = sx127x_RegWrite_RTOS_NoLock(chip->base_chip, sx127x_LORA_REG_0D_FIFO_ADDR_PTR, value);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }

	// Read the data from the fifo
	status = sx127x_RegReadMulti_RTOS_NoLock(chip->base_chip, sx127x_REG_00_FIFO, data, packetLength);
	if (status != sx127x_STATUS_OK) { goto exit_flag; }
