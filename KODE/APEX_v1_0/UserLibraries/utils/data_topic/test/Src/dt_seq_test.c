...
void DT_seq_test_t0_null_args(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_BAD_ARG, "publish(NULL,val) retourne %d != DT_BAD_ARG", s);

    s = data_topic_publish(&topic, NULL);
    DT_ASSERT(s == DT_BAD_ARG, "publish(topic,NULL) retourne %d != DT_BAD_ARG", s);

    /* data_sub_attach avec pointeurs NULL */
    data_sub_t sub = {0};
    s = data_sub_attach(NULL, &topic, DATA_ATTACH_FROM_NOW);
    DT_ASSERT(s == DT_BAD_ARG, "attach(NULL,topic) retourne %d != DT_BAD_ARG", s);

    s = data_sub_attach(&sub, NULL, DATA_ATTACH_FROM_NOW);
    DT_ASSERT(s == DT_BAD_ARG, "attach(sub,NULL) retourne %d != DT_BAD_ARG", s);

    /* data_sub_detach sur abonné non attaché ou NULL */
    s = data_sub_detach(NULL);
    DT_ASSERT(s == DT_BAD_ARG, "detach(NULL) retourne %d != DT_BAD_ARG", s);
    /* sub.attached == 0 */
    s = data_sub_detach(&sub);
    DT_ASSERT(s == DT_BAD_ARG, "detach(non-attache) retourne %d != DT_BAD_ARG", s);

    /* data_sub_sync sur NULL ou non attaché */
    s = data_sub_sync(NULL);
    DT_ASSERT(s == DT_BAD_ARG, "sync(NULL) retourne %d != DT_BAD_ARG", s);
    s = data_sub_sync(&sub);
    DT_ASSERT(s == DT_BAD_ARG, "sync(non-attache) retourne %d != DT_BAD_ARG", s);

    /* data_sub_read avec NULL */
    uint32_t out;
    s = data_sub_read(NULL, &out);
    DT_ASSERT(s == DT_BAD_ARG, "read(NULL,out) retourne %d != DT_BAD_ARG", s);

    /* data_sub_read sur non attaché */
    s = data_sub_read(&sub, &out);
    DT_ASSERT(s == DT_BAD_ARG, "read(non-attache,out) retourne %d != DT_BAD_ARG", s);
...
void DT_seq_test_t1_publish_read_fifo(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_OK, "attach retourne %d != DT_OK", s);

    uint32_t in[3] = {10u, 20u, 30u};
    for (int i = 0; i < 3; i++) {
        s = data_topic_publish(&topic, &in[i]);
        DT_ASSERT(s == DT_OK, "publish[%d] retourne %d != DT_OK", i, s);
    }
    DT_ASSERT(topic.pub_seq == 3u, "pub_seq=%u != 3", (unsigned)topic.pub_seq);
    DT_ASSERT(data_sub_num_to_read(&sub) == 3u,
              "num_to_read=%u != 3 apres 3 publish", (unsigned)data_sub_num_to_read(&sub));

    for (int i = 0; i < 3; i++) {
        uint32_t out = 0u;
        s = data_sub_read(&sub, &out);
        DT_ASSERT(s   == DT_OK,  "read[%d] retourne %d != DT_OK", i, s);
        DT_ASSERT(out == in[i],  "read[%d]=%u attendu %u", i, (unsigned)out, (unsigned)in[i]);
    }
    DT_ASSERT(data_sub_num_to_read(&sub) == 0u,
...
void DT_seq_test_t2_empty_read(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_EMPTY, "read sur vide retourne %d != DT_EMPTY", s);
    DT_ASSERT(out == 0xDEADBEEFu, "out modifié alors que DT_EMPTY (out=0x%08X)", (unsigned)out);

    s = data_sub_peek(&sub, &out, 0);
    DT_ASSERT(s == DT_EMPTY, "peek sur vide retourne %d != DT_EMPTY", s);
...
void DT_seq_test_t3_attach_from_oldest(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_OK, "attach FROM_OLDEST retourne %d != DT_OK", s);
    DT_ASSERT(data_sub_num_to_read(&sub) == 3u,
    ...
    DT_ASSERT(s == DT_EMPTY, "4eme read retourne %d != DT_EMPTY", s);
...
void DT_seq_test_t4_attach_from_now(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_EMPTY, "read avant any new pub retourne %d != DT_EMPTY", s);

    /* Publie une nouvelle valeur : l'abonné doit la voir */
    uint32_t v40 = 40u;
    data_topic_publish(&topic, &v40);
    DT_ASSERT(data_sub_num_to_read(&sub) == 1u,
              "num_to_read=%u != 1 apres pub 40", (unsigned)data_sub_num_to_read(&sub));

    uint32_t out = 0u;
    s = data_sub_read(&sub, &out);
    DT_ASSERT(s   == DT_OK,  "read apres pub 40 retourne %d != DT_OK", s);
    DT_ASSERT(out == 40u,    "read=%u attendu 40", (unsigned)out);
...
void DT_seq_test_t5_num_to_read(TEST_case_t *tc) {
    ...
    DT_ASSERT(data_sub_num_to_read(&sub) == 1u, "[1pub] num=%u != 1",
              (unsigned)data_sub_num_to_read(&sub));

    v = 2u; data_topic_publish(&topic, &v);
    DT_ASSERT(data_sub_num_to_read(&sub) == 2u, "[2pub] num=%u != 2",
              (unsigned)data_sub_num_to_read(&sub));

    v = 3u; data_topic_publish(&topic, &v);
    DT_ASSERT(data_sub_num_to_read(&sub) == 3u, "[3pub] num=%u != 3",
              (unsigned)data_sub_num_to_read(&sub));
    ...
    for (uint32_t expected = 2u; expected != (uint32_t)-1u; expected--) {
        ...
        DT_ASSERT(n == expected, "apres read: num=%u attendu %u",
...
void DT_seq_test_t6_two_subscribers(TEST_case_t *tc) {
    ...
    DT_ASSERT(topic.sub_count == 2u, "sub_count=%u != 2", (unsigned)topic.sub_count);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        DT_ASSERT(s   == DT_OK,  "[sub1] read[%d] retourne %d != DT_OK", i, s);
        DT_ASSERT(out == in[i],  "[sub1] read[%d]=%u attendu %u",
                  i, (unsigned)out, (unsigned)in[i]);
    }

    /* sub2 ne doit pas avoir avancé */
    DT_ASSERT(data_sub_num_to_read(&sub2) == 3u,
              "[sub2] num=%u != 3 (avance par sub1?)", (unsigned)data_sub_num_to_read(&sub2));
    DT_ASSERT(data_sub_num_to_read(&sub1) == 1u,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        DT_ASSERT(s   == DT_OK,  "[sub2] read[%d] retourne %d != DT_OK", i, s);
        DT_ASSERT(out == in[i],  "[sub2] read[%d]=%u attendu %u",
                  i, (unsigned)out, (unsigned)in[i]);
    }
    DT_ASSERT(data_sub_num_to_read(&sub2) == 0u,
...
#define CAP7 3u
    ...
    DT_ASSERT(data_sub_num_to_read(&sub) > CAP7,
    ...
    uint32_t out = 0u;
    s = data_sub_read(&sub, &out);
    DT_ASSERT(s   == DT_OK, "read post-sync retourne %d != DT_OK", s);
    DT_ASSERT(out == 99u,   "read post-sync=%u attendu 99", (unsigned)out);
...
void DT_seq_test_t8_sync(TEST_case_t *tc) {
    ...
    s = data_sub_read(&sub, &out);
    DT_ASSERT(s   == DT_OK, "read post-sync retourne %d != DT_OK", s);
    DT_ASSERT(out == 40u,   "read post-sync=%u attendu 40", (unsigned)out);
...
void DT_seq_test_t9_detach_reattach(TEST_case_t *tc) {
    ...
    data_sub_attach(&sub1, &topic, DATA_ATTACH_FROM_NOW);
    data_sub_attach(&sub2, &topic, DATA_ATTACH_FROM_NOW);
    DT_ASSERT(topic.sub_count == 2u, "sub_count=%u != 2 apres 2 attach",
              (unsigned)topic.sub_count);

    /* Détache sub1 */
    data_status_t s = data_sub_detach(&sub1);
    DT_ASSERT(s == DT_OK, "detach sub1 retourne %d != DT_OK", s);
    DT_ASSERT(topic.sub_count == 1u, "sub_count=%u != 1 apres detach",
              (unsigned)topic.sub_count);
    DT_ASSERT(sub1.attached == 0, "sub1.attached=%d != 0", sub1.attached);

    /* sub1 détaché ne peut pas lire */
    s = data_sub_read(&sub1, &(uint32_t){0u});
    DT_ASSERT(s == DT_BAD_ARG, "read sur sub1 detache retourne %d != DT_BAD_ARG", s);
    ...
    DT_ASSERT(s   == DT_OK, "sub2 read retourne %d != DT_OK", s);
    DT_ASSERT(out == 99u,   "sub2 read=%u attendu 99", (unsigned)out);

    /* Re-attache sub1 */
    s = data_sub_attach(&sub1, &topic, DATA_ATTACH_FROM_NOW);
    DT_ASSERT(s == DT_OK, "re-attach sub1 retourne %d != DT_OK", s);
    DT_ASSERT(topic.sub_count == 2u, "sub_count=%u != 2 apres re-attach",
              (unsigned)topic.sub_count);
    DT_ASSERT(data_sub_num_to_read(&sub1) == 0u,
...
void DT_seq_test_t10_peek_non_destructive(TEST_case_t *tc) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        DT_ASSERT(s   == DT_OK,        "peek[%d] retourne %d != DT_OK", i, s);
        DT_ASSERT(out == expected[i],   "peek[%d]=%u attendu %u",
                  i, (unsigned)out, (unsigned)expected[i]);
    }

    /* Non destructif : tail et num_to_read inchangés */
    DT_ASSERT(sub.tail == tail_before,
    ...
    uint32_t out = 0u;
    data_status_t s = data_sub_read(&sub, &out);
    DT_ASSERT(s   == DT_OK, "read apres peeks retourne %d != DT_OK", s);
    DT_ASSERT(out == 10u,   "read apres peeks=%u attendu 10 (tete FIFO)", (unsigned)out);
...
void DT_seq_test_t11_reject_new_policy(TEST_case_t *tc) {
    ...
    DT_ASSERT(s == DT_OK, "publish 10 retourne %d != DT_OK", s);
    v = 20u; s = data_topic_publish(&topic, &v);
    DT_ASSERT(s == DT_OK, "publish 20 retourne %d != DT_OK", s);

    DT_ASSERT(topic.pub_seq == 2u, "pub_seq=%u != 2 avant reject", (unsigned)topic.pub_seq);

    v = 30u; s = data_topic_publish(&topic, &v);
    DT_ASSERT(s == DT_FULL, "publish 30 retourne %d != DT_FULL (buffer plein)", s);

    /* pub_seq ne doit pas avoir incrementé (publication refusée) */
    DT_ASSERT(topic.pub_seq == 2u, "pub_seq=%u != 2 apres reject", (unsigned)topic.pub_seq);
    DT_ASSERT(data_sub_num_to_read(&sub) == 2u,
              "num_to_read=%u != 2", (unsigned)data_sub_num_to_read(&sub));

    /* Lecture : seulement {10, 20}, 30 non stocké */
    uint32_t out;
    out = 0u; s = data_sub_read(&sub, &out);
    DT_ASSERT(s == DT_OK && out == 10u, "1er read: s=%d out=%u attendu 10", s, (unsigned)out);
    out = 0u; s = data_sub_read(&sub, &out);
    DT_ASSERT(s == DT_OK && out == 20u, "2eme read: s=%d out=%u attendu 20", s, (unsigned)out);
    out = 0u; s = data_sub_read(&sub, &out);
    DT_ASSERT(s == DT_EMPTY, "3eme read retourne %d != DT_EMPTY", s);
