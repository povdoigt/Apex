...
float Waveform_Play_Scalar(waveform_t *waveform, uint32_t current_time) {
    if (waveform->kind != WAVEFORM_SCALAR) {
...
float3_t Waveform_Play_Space(waveform_t *waveform, uint32_t current_time) {
    if (waveform->kind != WAVEFORM_SPACE) {
        return FLOAT3_ZERO;
    }
    if (!Waveform_IsActive(waveform)) {
        return FLOAT3_ZERO;
